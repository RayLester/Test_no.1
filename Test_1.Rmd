...
library(pacman)
p_load(tidyverse, haven, psych, vcd, magrittr, forcats, car, sjmisc, glm)
...
Question 2
```{r}
#First we will explore the data in relation to the conviction data across ages 10-24.
...
set.seed(2)
train.dat <- dplyr::sample_frac(convicted_y_n_joined.dat, 0.75)
test.dat <- dplyr::setdiff(convicted_y_n_joined.dat, train.dat)

convict.mod1 <- train.dat %>% 
  glm(convicted_10_24 ~., family = "binomial")
summary(convict.mod1)
1- pchisq(convict.mod1$deviance, convict.mod1$df.residual)

#How much does separation from a mother add
convict.modboth <- train.dat %>% 
  glm(convicted_10_24 ~ sep_father_10 + sep_mother_10 + temp_sep_father_10 + temp_sep_mother_10, family = "binomial")

convict.modfather <-train.dat %>% 
  glm(convicted_10_24~sep_father_10 + temp_sep_father_10, family = "binomial")

nestedanova1 <- anova(convict.modboth, convict.modfather)
lrtest(convict.modboth, convict.modfather)

#How much does temporary separation add
convict.modallsep <- train.dat %>% 
  glm(convicted_10_24 ~ broken_home + sep_father_10 + sep_mother_10 + temp_sep_father_10 + temp_sep_mother_10, family = "binomial")

convict.modlongsep <- train.dat %>% 
  glm(convicted_10_24 ~ broken_home + sep_father_10 + sep_mother_10, family = "binomial")

nestedanova2 <- anova(convict.modallsep, convict.modlongsep)
